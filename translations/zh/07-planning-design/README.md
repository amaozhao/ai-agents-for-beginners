...
这种模块化方法还允许逐步改进。例如，你可以添加专门的代理来提供餐饮推荐或本地活动建议，并随着时间推移不断优化行程。
...
上面代码的输出如下，你可以使用此结构化输出路由到 `assigned_agent` 并将旅行计划总结给最终用户。
...
在本文中，我们展示了如何创建一个规划器，该规划器可以动态选择定义的可用代理。规划器的输出将任务分解并分配给代理以执行。假设代理可以访问完成任务所需的功能/工具。除了代理之外，你还可以包括其他模式，例如反思、总结器和轮询聊天，以进一步定制。
...
加入 [Azure AI Foundry Discord](https://aka.ms/ai-agents/discord)，与其他学习者交流，参加答疑时间，并获取你的 AI 代理相关问题的解答。
