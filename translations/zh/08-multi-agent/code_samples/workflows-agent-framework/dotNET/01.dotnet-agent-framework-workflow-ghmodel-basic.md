...
本教程展示了如何使用 Microsoft Agent Framework for .NET 和 GitHub 模型构建复杂的 **代理工作流**。你将学习如何创建多步骤业务流程，让 AI 代理通过结构化编排模式协作完成复杂任务。
