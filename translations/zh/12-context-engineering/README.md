...
理解你为其构建 AI 代理的应用程序的复杂性，对于创建一个可靠的代理至关重要。我们需要构建能够有效管理信息的 AI 代理，以满足超越提示工程的复杂需求。
...
• **常见的上下文问题**，这些问题可能会使你的 AI 代理偏离轨道，以及如何解决这些问题。
...
良好的上下文工程始于良好的规划。以下是帮助你开始思考如何应用上下文工程概念的方法：

1. **定义清晰的结果** - AI 代理将被分配的任务结果应清晰定义。回答这个问题——“当 AI 代理完成任务时，世界会是什么样子？”换句话说，用户在与 AI 代理交互后应获得什么样的变化、信息或响应。

2. **绘制上下文图** - 一旦定义了 AI 代理的任务结果，你需要回答“AI 代理需要什么信息来完成这项任务？”这样，你可以开始绘制这些信息的来源。

3. **创建上下文管道** - 知道信息来源后，你需要回答“代理将如何获取这些信息？”这可以通过多种方式实现，包括 RAG、使用 MCP 服务器和其他工具。
...
• 一个仅使用提示工程的简单代理可能只会回答：**“好的，你想什么时候去巴黎？”** 它只处理了用户在提问时的直接问题。

• 一个使用本课中提到的上下文工程策略的代理会做得更多。在回答之前，它的系统可能会：

  ◦ **检查你的日历** 以获取可用日期（实时数据检索）。  
  ◦ **回忆过去的旅行偏好**（来自长期记忆），例如你喜欢的航空公司、预算或是否偏好直飞航班。  
  ◦ **识别可用工具** 用于预订航班和酒店。  

- 然后，一个示例响应可能是：  
  “你好 [你的名字]！我看到你在十月的第一周有空。我是否可以帮你查找 [你喜欢的航空公司] 的直飞巴黎航班，预算控制在 [预算] 内？”  
...
**旅行预订示例：** 你的代理虚构了一个 **从小型地方机场直飞远程国际城市** 的航班，而实际上该机场并不提供国际航班。这一不存在的航班信息被保存到上下文中。之后，当你要求代理预订时，它不断尝试为这一不可能的航线寻找票务，导致重复错误。
...
**旅行预订示例：** 你长时间讨论了各种梦想旅行目的地，包括详细回忆两年前的背包旅行。当你最终要求 **“帮我找下个月的便宜航班”** 时，代理因旧的、不相关的细节而分心，不断询问你的背包装备或过去的行程，而忽略了当前的请求。
...
**旅行预订示例：** 你的代理可以访问几十种工具：`book_flight`、`book_hotel`、`rent_car`、`find_tours`、`currency_converter`、`weather_forecast`、`restaurant_reservations` 等。你询问 **“在巴黎最好的出行方式是什么？”** 由于工具数量过多，代理可能会混淆，尝试在巴黎内部调用 `book_flight`，或者调用 `rent_car`，尽管你更喜欢公共交通，因为工具描述可能存在重叠，或者代理无法分辨最佳选择。

**解决方案：** 使用 **RAG 技术筛选工具描述**。当你询问巴黎的出行方式时，系统动态检索 _仅_ 最相关的工具，如 `rent_car` 或 `public_transport_info`，为 LLM 提供一个精简的“工具加载”。
...
**旅行预订示例：** 你最初告诉代理 **“我想坐经济舱。”** 后来在对话中改变主意，说 **“其实这次我想坐商务舱。”** 如果两条指令都保留在上下文中，代理可能会收到冲突的搜索结果，或者在优先处理哪条指令时感到困惑。
