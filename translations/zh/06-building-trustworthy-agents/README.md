...
如果你曾使用大型语言模型（LLMs）构建AI应用程序，你会知道设计一个强大的系统提示或系统消息的重要性。这些提示设定了元规则、指令和指南，规定了LLM如何与用户和数据交互。
...
下一步是创建一个基础提示来描述AI代理。你应包括代理的角色、代理将完成的任务以及代理的其他职责。
...
此系统消息框架的价值在于能够更轻松地为多个代理扩展创建系统消息，并随着时间的推移改进你的系统消息。很少有系统消息能够在第一次就完全适用于你的用例。通过更改基础系统消息并运行系统，你可以进行小幅调整和改进，从而比较和评估结果。
...
加入[Azure AI Foundry Discord](https://aka.ms/ai-agents/discord)，与其他学习者交流，参加答疑时间并获取你的AI代理相关问题的解答。
